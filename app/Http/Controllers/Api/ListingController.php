...
class ListingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Log::info('ListingController@store payload', $request->all());

        // Ensure authenticated user is an owner and has an owner record
        $user = $request->user();
        if (! $user || $user->role !== 'owner' || ! $user->owner) {
            return response()->json(['message' => 'Only owners can create listings.'], 403);
        }

        // Validate all fields except owner_id; we'll assign owner_id from the authenticated user's owner record
        ...
        Log::info('ListingController@store created', ['listing_id' => $listing->id, 'owner_id' => $validatedData['owner_id']]);
